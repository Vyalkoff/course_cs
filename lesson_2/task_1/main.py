    ...
    for i in range(1, 4):
        with open(f'info_{i}.txt', 'r', encoding='utf-8') as file:
            for line in file:
                ...
                if main_data[0][1] in line:
                    os_name_list.append(' '.join(line.split()[2:]))
                if main_data[0][2] in line:
                    os_code_list.append(''.join(line.split()[2:]))
                if main_data[0][3] in line:
                    os_type_list.append(''.join(line.split()[2:-1]))

    for i in range(0, 3):
        section = [i+1, os_prod_list[i], os_name_list[i], os_code_list[i], os_type_list[i]]
        main_data.append(section)
